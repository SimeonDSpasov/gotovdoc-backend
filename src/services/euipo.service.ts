...
export default class EuipoService {
 ...
 public async seedClassDescriptions(): Promise<void> {
  ...
  logger.info(`Fetched ${headings.length} class headings, upserting descriptions`);

  for (const h of headings) {
   await this.cacheDataLayer.upsertDescription(h.classNumber, h.heading ?? '', logContext);
  }

  logger.info(`Seeded descriptions for ${headings.length} classes`);
 }
 ...
 public async syncAllClasses(): Promise<void> {
  ...
  logger.info(`Syncing all ${allClassNumbers.length} classes`);
  ...
  logger.info(`Sync complete: ${allClassNumbers.length} classes in ${elapsed}s`);
 }
 ...
 private async syncSingleClass(
  ...
  logger.info(`Class ${classNum}: ${allTerms.length} terms synced`);
 }
 ...
 */
 ...
 private handleRequestError<T>(
  ...
  if (status === 401) {
   ...
   if (attempt === 0) {
    logger.info('Received 401 from EUIPO, refreshing token and retrying');
    this.cachedToken = null;
  ...
  if (isTransient && attempt < this.MAX_RETRIES) {
   ...
   logger.info(`Transient error (${status || 'network'}): ${detail} — retrying in ${backoffMs / 1000}s (attempt ${attempt + 1}/${this.MAX_RETRIES})`);
 ...
 private log401Details(
  ...
  logger.info(`EUIPO 401 request URL: ${fullUrl}`);
  ...
  if (respHeaders && typeof respHeaders === 'object') {
   ...
   logger.info(`EUIPO 401 response headers: ${JSON.stringify(headerObj)}`);
  }

  if (err.response?.data) {
   logger.info(`EUIPO 401 response body: ${JSON.stringify(err.response.data)}`);
  }
 }
 ...
 private async fetchToken(): Promise<string> {
  ...
  if (needNewToken && stored?.refreshToken) {
   const result = await this.tryRefresh(stored.refreshToken, logContext).catch((err: any) => {
    logger.info(`Refresh failed, falling back to client_credentials: ${err.message}`);
    return null;
   });
   if (result) {
    ...
    logger.info('Token refreshed, expires in ' + (result.expiresIn ?? 0) + 's');
  ...
  logger.info(`Token acquired, expires in ${result.expiresIn}s`);
  return result.accessToken;
 }
 ...
 private async fetchNewToken(
  ...
  logger.info(`Token response keys: ${Object.keys(data || {}).join(', ')}`);
