...
export default class AuthMiddleware {
 ...
 public isAuthenticated: RequestHandler = async (req, res, next) => {
  ...
  await Promise.resolve()
   .then(() => this.tokenUtil.getUserIdFromAccessToken(accessToken))
   .then(userId => this.userDataLayer.getById(userId, logContext, ''))
   .then(user => {
    if (user.suspended) {
   ...
   .catch((err) => next(new CustomError(498, 'Expired token')));
 }
