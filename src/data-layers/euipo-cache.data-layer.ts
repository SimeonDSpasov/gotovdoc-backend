...
export default class EuipoCacheDataLayer {
 ...
 public async getByClassNumber(classNumber: number, logContext: string): Promise<IEuipoClass | null> {
  ...
  return EuipoClass.findOne({ classNumber })
   .lean()
   .catch((err: any) => {
 ...
 public async getAllClasses(logContext: string): Promise<ClassHeading[]> {
  ...
  return EuipoClass.find({}, { classNumber: 1, heading: 1, description: 1, totalTerms: 1, syncedAt: 1 })
   .sort({ classNumber: 1 })
   .lean()
   .then(docs =>
    docs.map(d => ({
 ...
 public async upsertClass(
  ...
  return EuipoClass.findOneAndUpdate(
   { classNumber },
 ...
 public async getClassDescriptions(logContext: string): Promise<{ classNumber: number; description: string }[]> {
  ...
  return EuipoClass.find({}, { classNumber: 1, description: 1 })
   .sort({ classNumber: 1 })
   .lean()
   .then(docs =>
    docs.map(d => ({
 ...
 public async getClassTerms(
  ...
  return EuipoClass.aggregate(pipeline)
   .then(([result]) => {
 ...
 public async getStaleClasses(maxAgeMs: number, logContext: string): Promise<number[]> {
  ...
  return EuipoClass.find(
   { $or: [{ syncedAt: null }, { syncedAt: { $lt: cutoff } }] },
   { classNumber: 1 },
  )
   .lean()
   .then(staleDocs =>
    EuipoClass.find({}, { classNumber: 1 })
     .lean()
     .then(allDocs => {
 ...
 public async getStats(logContext: string): Promise<{ classNumber: number; totalTerms: number; syncedAt: Date | null }[]> {
  ...
  return EuipoClass.find({}, { classNumber: 1, totalTerms: 1, syncedAt: 1 })
   .sort({ classNumber: 1 })
   .lean()
   .then(docs =>
    docs.map(d => ({
